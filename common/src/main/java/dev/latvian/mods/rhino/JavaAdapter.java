	...
	static Object js_createAdapter(Context cx, Scriptable scope, Object[] args) {
		...
		for (classCount = 0; classCount < N - 1; classCount++) {
			...
			if (!(arg instanceof NativeJavaClass)) {
				throw ScriptRuntime.typeError2("msg.not.java.class.arg", String.valueOf(classCount), ScriptRuntime.toString(cx, arg));
			}
		}
			...
			if (self instanceof Wrapper) {
				...
				if (unwrapped instanceof Scriptable) {
					if (unwrapped instanceof ScriptableObject) {
						ScriptRuntime.setObjectProtoAndParent(cx, (ScriptableObject) unwrapped, scope);
					}
					return unwrapped;
				}
			}
	...
	private static ObjToIntMap getObjectFunctionNames(Context cx, Scriptable obj) {
		...
		for (int i = 0; i != ids.length; ++i) {
			...
			if (value instanceof Function f) {
				int length = ScriptRuntime.toInt32(cx, ScriptableObject.getProperty(cx, f, "length"));
				if (length < 0) {
					length = 0;
				}
				map.put(id, length);
