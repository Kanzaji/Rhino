...
final class NativeString extends IdScriptableObject implements Wrapper {
	...
	protected void initPrototypeId(Context cx, int id) {
		if (id == SymbolId_iterator) {
			initPrototypeMethod(cx, STRING_TAG, id, SymbolKey.ITERATOR, "[Symbol.iterator]", 0);
			return;
		}
		...
		initPrototypeMethod(cx, STRING_TAG, id, s, fnName, arity);
	}
		...
		for (; ; ) {
			switch (id) {
				...
				case ConstructorId_fromCodePoint: {
					...
					for (int i = 0; i != n; i++) {
						...
						if (!ScriptRuntime.eqNumber(cx, num, codePoint) || !Character.isValidCodePoint(codePoint)) {
							throw ScriptRuntime.rangeError("Invalid code point " + ScriptRuntime.toString(cx, arg));
						}
				...
				case Id_indexOf: {
					...
					return js_indexOf(cx, Id_indexOf, thisString, args);
				}
				...
				case Id_endsWith:
					...
					int idx = js_indexOf(cx, id, thisString, args);
				...
				case Id_lastIndexOf: {
					...
					return js_lastIndexOf(cx, thisStr, args);
				}
				...
				case Id_bold:
					return tagify(cx, thisObj, "b", null, null);

				case Id_italics:
					return tagify(cx, thisObj, "i", null, null);

				case Id_fixed:
					return tagify(cx, thisObj, "tt", null, null);

				case Id_strike:
					return tagify(cx, thisObj, "strike", null, null);

				case Id_small:
					return tagify(cx, thisObj, "small", null, null);

				case Id_big:
					return tagify(cx, thisObj, "big", null, null);

				case Id_blink:
					return tagify(cx, thisObj, "blink", null, null);

				case Id_sup:
					return tagify(cx, thisObj, "sup", null, null);

				case Id_sub:
					return tagify(cx, thisObj, "sub", null, null);

				case Id_fontsize:
					return tagify(cx, thisObj, "font", "size", args);

				case Id_fontcolor:
					return tagify(cx, thisObj, "font", "color", args);

				case Id_link:
					return tagify(cx, thisObj, "a", "href", args);

				case Id_anchor:
					return tagify(cx, thisObj, "a", "name", args);
	 ...
	 */
	private static String tagify(Context cx, Scriptable thisObj, String tag, String attribute, Object[] args) {
	 ...
	 */
	private static int js_indexOf(Context cx, int methodId, String target, Object[] args) {
	 ...
	 *
	 */
	private static int js_lastIndexOf(Context cx, String target, Object[] args) {
